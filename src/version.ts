...
import { getHudPluginDir } from './antigravity-config-dir.js';
...
function resolveClaudeBinaryFromPath(): ClaudeBinaryInfo | null {
  ...
  // Look for 'agy' first, then 'claude'
  for (const binName of ['agy', 'claude']) {
    const candidates = getPathCandidates(binName);
