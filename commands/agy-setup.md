---
description: Configure agy-hud as your statusline
...
CACHE_EXISTS=$(ls -d "$ANTIGRAVITY_DIR/plugins/cache"/*/agy-hud 2>/dev/null && echo "YES" || echo "NO")

# Check 2: Registry entry exists?
REGISTRY_EXISTS=$(grep -q "agy-hud" "$ANTIGRAVITY_DIR/plugins/installed_plugins.json" 2>/dev/null && echo "YES" || echo "NO")

# Check 3: Temp files left behind?
TEMP_FILES=$(ls -d "$ANTIGRAVITY_DIR/plugins/cache/temp_local_"* 2>/dev/null | head -1)
...
**Windows (PowerShell)**:
```powershell
$antigravityDir = if ($env:ANTIGRAVITY_CONFIG_DIR) { $env:ANTIGRAVITY_CONFIG_DIR } else { Join-Path $HOME ".gemini/antigravity-cli" }
$cache = (Get-ChildItem (Join-Path $antigravityDir "plugins\cache") -Directory | ForEach-Object { Test-Path (Join-Path $_.FullName "agy-hud") }) -contains $true
$registry = (Get-Content (Join-Path $antigravityDir "plugins\installed_plugins.json") -ErrorAction SilentlyContinue) -match "agy-hud"
$temp = Get-ChildItem (Join-Path $antigravityDir "plugins\cache\temp_local_*") -ErrorAction SilentlyContinue
...
rm -rf "$ANTIGRAVITY_DIR/plugins/cache"/*/agy-hud

# Remove temp files from failed installs
rm -rf "$ANTIGRAVITY_DIR/plugins/cache/temp_local_"*

# Reset registry (removes ALL plugins - warn user first!)
# Only run if user confirms they have no other plugins they want to keep:
echo '{"version": 2, "plugins": {}}' > "$ANTIGRAVITY_DIR/plugins/installed_plugins.json"
```
...
# Remove orphaned cache (handles both direct and marketplace installs)
Get-ChildItem (Join-Path $antigravityDir "plugins\cache") -Directory | ForEach-Object { Remove-Item -Recurse -Force (Join-Path $_.FullName "agy-hud") -ErrorAction SilentlyContinue }

# Remove temp files
Remove-Item -Recurse -Force (Join-Path $antigravityDir "plugins\cache\temp_local_*") -ErrorAction SilentlyContinue

# Reset registry (removes ALL plugins - warn user first!)
'{"version": 2, "plugins": {}}' | Set-Content (Join-Path $antigravityDir "plugins\installed_plugins.json")
```

After cleanup, tell user to **restart Antigravity CLI** and run `/plugin install agy-hud` again.
...
```bash
mkdir -p ~/.cache/tmp && TMPDIR=~/.cache/tmp claude /plugin install agy-hud
```

This is a [Antigravity CLI platform limitation](https://github.com/anthropics/claude-code/issues/14799).

---

## Step 1: Detect Platform, Shell, and Runtime

**IMPORTANT**: Use the environment context values (`Platform:` and `Shell:`) as your starting point. On `win32`, also check `$OSTYPE` via the Bash tool. Some Windows sessions report `Shell: powershell` while the command path exposed to Antigravity CLI is Git Bash/MSYS2. When `$OSTYPE` is `msys` or `cygwin`, the PowerShell command format can fail before PowerShell runs because bash expands `$env:VAR`, `$p`, and `$(...)` expressions first (see [#531](https://github.com/jarrodwatts/agy-hud/issues/531)).
...
1. Get plugin path (sorted by dotted numeric version, not modification time):
   ```bash
   ls -d "${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}"/plugins/cache/*/agy-hud/*/ 2>/dev/null | awk -F/ '{ print $(NF-1) "\t" $(0) }' | grep -E '^[0-9]+\.[0-9]+\.[0-9]+[[:space:]]' | sort -t. -k1,1n -k2,2n -k3,3n -k4,4n | tail -1 | cut -f2-
   ```
   If empty, the plugin is not installed. Go back to Step 0 to check for ghost installation or EXDEV issues. If Step 0 was clean, ask the user to install via `/plugin install agy-hud` first.
...
2. Get runtime absolute path:
   ...
   - On **Windows + Git Bash/MSYS2**, explicitly explain that the current Git Bash session could not find Node.js, even if Antigravity CLI itself is installed.
   ...
   - After installation, ask the user to restart their shell and re-run `/agy-hud:setup`.
...
5. Generate command (quotes around runtime path handle spaces):
   ...
   Antigravity CLI pipes the subprocess stdout, so `process.stdout.columns` is
   unavailable at runtime. `stty size </dev/tty` reads from the controlling
   terminal. The `- 4` accounts for Antigravity CLI's input area padding
   ...
   **When runtime is bun** - add `--env-file /dev/null` to prevent Bun from auto-loading project `.env` files:
   ```
   bash -c 'cols=$(stty size </dev/tty 2>/dev/null | awk '"'"'{print $2}'"'"'); export COLUMNS=$(( ${cols:-120} > 4 ? ${cols:-120} - 4 : 1 )); plugin_dir=$(ls -d "${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}"/plugins/cache/*/agy-hud/*/ 2>/dev/null | awk -F/ '"'"'{ print $(NF-1) "\t" $(0) }'"'"' | grep -E '"'"'^[0-9]+\.[0-9]+\.[0-9]+[[:space:]]'"'"' | sort -t. -k1,1n -k2,2n -k3,3n -k4,4n | tail -1 | cut -f2-); exec "{RUNTIME_PATH}" --env-file /dev/null "${plugin_dir}{SOURCE}"'
   ```
...
Do not use PowerShell commands when the shell is bash. Antigravity CLI invokes statusLine commands through bash, which will interpret PowerShell variables like `$env` and `$p` before PowerShell ever sees them.

On Windows require `node` and always use `dist/index.js`.

**Important**: Do **not** reuse the macOS/Linux awk-based command on Windows + Git Bash. The `awk` fragment requires `'"'"'` quoting to nest single quotes inside `bash -c '...'`. After JSON encoding and decoding, this quoting breaks on Windows Git Bash, causing a silent syntax error that prevents the HUD process from starting (see [#326](https://github.com/jarrodwatts/agy-hud/issues/326)).

Instead, use `sort -V` (GNU version sort, included with Git for Windows) which avoids nested single quotes entirely. Also avoid wrapping the generated command in a second `bash -c ...` layer. Antigravity CLI is already invoking the statusline through bash, so the direct shell command lets `exec` replace that shell instead of spawning an extra bash wrapper first. The command still exports `COLUMNS` so the HUD receives the real terminal width, and it uses the marketplace-aware cache glob:

   ```
   cols=$(stty size </dev/tty 2>/dev/null | awk '{print $2}'); export COLUMNS=$(( ${cols:-120} > 4 ? ${cols:-120} - 4 : 1 )); plugin_dir=$(ls -1d "${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}"/plugins/cache/*/agy-hud/*/ 2>/dev/null | sort -V | tail -1); exec "{RUNTIME_PATH}" "${plugin_dir}{SOURCE}"
   ```
...
1. Get plugin path:
   ```powershell
   $antigravityDir = if ($env:ANTIGRAVITY_CONFIG_DIR) { $env:ANTIGRAVITY_CONFIG_DIR } else { Join-Path $HOME ".gemini/antigravity-cli" }
   (Get-ChildItem (Join-Path $antigravityDir "plugins\cache\*\agy-hud\*") -Directory -ErrorAction SilentlyContinue | Where-Object { $_.Name -match '^\d+(\.\d+)+$' } | Sort-Object { [version]$_.Name } -Descending | Select-Object -First 1).FullName
   ```
   The trailing `\*` on the cache glob is required. Without it, `Get-ChildItem` returns the `agy-hud` directory itself, whose name does not match the `^\d+(\.\d+)+$` version pattern, so the lookup resolves to `$null` and any subsequent `Join-Path` throws (see [#521](https://github.com/jarrodwatts/agy-hud/issues/521)).
...
2. Get runtime absolute path (require node on Windows):
   ...
   - If `winget` is available, recommend:
     ...
     winget install OpenJS.NodeJS.LTS
     ```
   - Otherwise ask the user to install Node.js LTS, then restart PowerShell and re-run `/agy-hud:setup`.
...
4. Write the PowerShell wrapper script.
   ...
   The wrapper file at `$antigravityDir/plugins/agy-hud/statusline.ps1` should contain:

   ```powershell
   try { $w = [Console]::WindowWidth } catch { $w = 120 }
   $env:COLUMNS = [Math]::Max(1, $w - 4)
   $antigravityDir = if ($env:ANTIGRAVITY_CONFIG_DIR) { $env:ANTIGRAVITY_CONFIG_DIR } else { Join-Path $HOME '.gemini/antigravity-cli' }
   $pluginDir = (Get-ChildItem (Join-Path $antigravityDir 'plugins\cache\*\agy-hud\*') -Directory -ErrorAction SilentlyContinue |
   ...
   ```powershell
   $wrapperDir = Join-Path $antigravityDir "plugins\agy-hud"
...
5. Generate command (points at the wrapper file, not an inline `-Command` string):
   ...
   `{WRAPPER_PATH}` is the value of `$wrapperPath` from step 4 (typically `C:\Users\<user>\.gemini/antigravity-cli\plugins\agy-hud\statusline.ps1`).

**WSL (Windows Subsystem for Linux)**: If running in WSL, use the macOS/Linux instructions. Ensure the plugin is installed in the Linux environment (`${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}/plugins/...`), not the Windows side.
...
- **Platform `darwin` or `linux`, or Platform `win32` + Shell `bash`**: `${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}/settings.json`
- **Platform `win32` + Shell `powershell`, `pwsh`, or `cmd`**: `settings.json` inside `$env:ANTIGRAVITY_CONFIG_DIR` when set, otherwise `Join-Path $HOME ".gemini/antigravity-cli"`
...
> ✅ Config written. **Please restart Antigravity CLI now** — quit and run `claude` again in your terminal.
> Once restarted, run `/agy-hud:setup` again to complete Step 4 and verify the HUD is working.
...
- After `statusLine` is written successfully, they should fully quit Antigravity CLI and launch a fresh session before judging whether the HUD setup worked.

**Note**: The generated command dynamically finds and runs the latest installed plugin version. Updates are automatic - no need to re-run setup after plugin updates. If the HUD suddenly stops working, re-run `/agy-hud:setup` to verify the plugin is still installed.
...
**If user selects any options**, write `plugins/agy-hud/config.json` inside the Antigravity config directory (`${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}` on bash, `$env:ANTIGRAVITY_CONFIG_DIR` or `Join-Path $HOME ".gemini/antigravity-cli"` on PowerShell). Create directories if needed:
...
**First, confirm the user has restarted Antigravity CLI** since Step 3 wrote the config. If they haven't, ask them to restart before proceeding — the HUD cannot appear in the same session where setup was run.
...
**If yes**: Ask the user if they'd like to ⭐ star the agy-hud repository on GitHub to support the project. If they agree and `gh` CLI is available, first check whether their `gh` version supports `gh repo star`. If it does, run `gh repo star jarrodwatts/agy-hud`. Otherwise fall back to `gh api -X PUT /user/starred/jarrodwatts/agy-hud`. Only run the star command if they explicitly say yes.

**If no**: Debug systematically:

1. **Restart Antigravity CLI** (most common cause on macOS):
    - The statusLine config requires a restart to take effect
    - Quit Antigravity CLI completely and run `claude` again, then re-run `/agy-hud:setup` to verify
    - If you've already restarted, continue below

2. **Verify config was applied**:
   - Read settings file (`${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}/settings.json` on bash, or `settings.json` inside `$env:ANTIGRAVITY_CONFIG_DIR` when set, otherwise `Join-Path $HOME ".gemini/antigravity-cli"` on PowerShell)
...
4. **Common issues to check**:
   ...
   **"No such file or directory" for plugin**:
   - Plugin might not be installed: `ls "${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}"/plugins/cache/*/agy-hud/`
   ...
   **Windows + PowerShell: HUD silent or "initializing..." with no error in any log (OSTYPE is not msys/cygwin)**:
   - Symptoms: HUD stays at "initializing..." or shows nothing. Running the generated command interactively in a PowerShell prompt produces the expected HUD output, but the version invoked through Antigravity CLI does not.
   - Root cause: either (a) `[Console]::WindowWidth` threw `System.IO.IOException: The handle is invalid.` because the subprocess Antigravity CLI spawns has no console handle, or (b) the cache glob `plugins\cache\*\agy-hud` (with no trailing `\*`) matched the `agy-hud` directory itself, leaving `$pluginDir` as `$null` and `Join-Path` throwing `Cannot bind argument to parameter 'Path' because it is null`.
   - Check: pipe stdin through `cmd.exe` to mirror Antigravity CLI's invocation:
     ```powershell
     '{}' | & cmd.exe /c '{GENERATED_COMMAND}'
     ```
     If you see either error, the existing setup predates the wrapper-based command format. Re-run `/agy-hud:setup` to regenerate `statusline.ps1` with `try/catch` and the corrected version-dir glob. See [#521](https://github.com/jarrodwatts/agy-hud/issues/521).
   ...
   **WSL confusion**:
   ...
   - Check: `ls "${ANTIGRAVITY_CONFIG_DIR:-$HOME/.gemini/antigravity-cli}"/plugins/cache/*/agy-hud/`
