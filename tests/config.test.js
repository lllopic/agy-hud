...
test('getConfigPath returns correct path', () => {
  const originalConfigDir = process.env.ANTIGRAVITY_CONFIG_DIR;
  ...
  try {
    ...
    assert.equal(configPath, path.join(homeDir, '.gemini', 'antigravity-cli', 'plugins', 'agy-hud', 'config.json'));
  } finally {
    restoreEnvVar('ANTIGRAVITY_CONFIG_DIR', originalConfigDir);
